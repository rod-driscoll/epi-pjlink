        ...
        IWarmingCooling, IOnline, IBasicVideoMuteWithFeedback, ICommunicationMonitor, IHasFeedback, IBasicVolumeControls
    {
        ...
        public override bool CustomActivate()
        {
            ...
            _pollTimer = new CTimer(o =>
                {
                    Debug.LogMessage(LogEventLevel.Verbose, this, "Polling, IsOnline: {0}, Status: {1}, IsConnected: {2}, ", CommunicationMonitor.IsOnlineFeedback.BoolValue, CommunicationMonitor.Status, _coms.IsConnected);
                    ...
                    if (ClassString.Equals(Commands.Protocol2))
                    {
                        ...
                        {
                            Coms = _coms,
                            Message = MakeQueryCommand(Commands.Freeze, ClassString)
                        });
                    }
                },
                null,
        ...
        public string MakeCommand(string command, string classString)
        {
            var msg_ = String.Format("{0}{1}{2}", AuthString, classString, command);
            //Debug.LogMessage(LogEventLevel.Verbose, this, "MakeCommand msg_: {0}", msg_);
            return msg_;
        }
        public string MakeCommand(string command)
        {
            return MakeCommand(command, Commands.Protocol1);
        }
        public string MakeQueryCommand(string command, string classString)
        {
            //Debug.LogMessage(LogEventLevel.Verbose, this, "MakeQueryCommand: {0}:{1}:{2}:{3}", AuthString, ClassString, command, Commands.Query);
            var msg_ = String.Format("{0}{1}{2}{3}", AuthString, classString, command, Commands.Query);
            //Debug.LogMessage(LogEventLevel.Verbose, this, "MakeQueryCommand msg_: '{0}'", msg_);
            return msg_;
        }
        public string MakeQueryCommand(string command)
        {
            return MakeQueryCommand(command, Commands.Protocol1);
        }
        ...
        {
            Debug.LogMessage(LogEventLevel.Verbose, this, "HandlePowerStatusUpdated Status: '{0}'", eventArgs.Status);
        ...
        {
            Debug.LogMessage(LogEventLevel.Verbose, this, "ProcessRequestedPowerStatus() _requestedPowerStatus: {0}", _requestedPowerStatus.ToString());
            switch (_requestedPowerStatus)
            {
                ...
                case PowerHandler.PowerStatusEnum.Error:
                    break;
                default:
                    Debug.LogMessage(LogEventLevel.Debug, this, "ProcessRequestedPowerStatus() ERROR, _requestedPowerStatus: {0}, not handled", _requestedPowerStatus.ToString());
        ...
        private void ProcessRequestedPowerOn()
        {
            if (_requestedPowerStatus != PowerHandler.PowerStatusEnum.PowerOn)
                Debug.LogMessage(LogEventLevel.Debug, this, "ProcessRequestedPowerOn() Power on isn't requested. Current request: {0}", _requestedPowerStatus.ToString());
            ...
            switch (_currentPowerStatus)
            {
                ...
                case PowerHandler.PowerStatusEnum.None:
                    break;
                default:
                    Debug.LogMessage(LogEventLevel.Debug, this, "ProcessRequestedPowerOn() ERROR, _requestedPowerStatus: {0}, not handled", _requestedPowerStatus.ToString());
                    //throw new ArgumentOutOfRangeException();
                    break;
            }
            ...
            {
                Coms = _coms,
                Message = MakeCommand(Commands.Power + " " + Commands.On), // "%1POWR ON"
            });
        }

        private void ProcessRequestedPowerOff()
        {
            //Debug.LogMessage(LogEventLevel.Verbose, this, "ProcessRequestedPowerOff()");
            if (_requestedPowerStatus != PowerHandler.PowerStatusEnum.PowerStandby)
                Debug.LogMessage(LogEventLevel.Debug, this, "ProcessRequestedPowerOff() Power off isn't requested. Current request: {0}", _requestedPowerStatus.ToString());
                //throw new InvalidOperationException("Power off isn't requested");

            //Debug.LogMessage(LogEventLevel.Debug, this, "ProcessRequestedPowerOff() _currentPowerStatus: {0}", _currentPowerStatus.ToString());
            switch (_currentPowerStatus)
            {
                case PowerHandler.PowerStatusEnum.PowerOn:
                ...
                case PowerHandler.PowerStatusEnum.None:
                    break;
                default:
                    Debug.LogMessage(LogEventLevel.Debug, this, "ProcessRequestedPowerOff() ERROR _currentPowerStatus: {0}, not handled", _currentPowerStatus.ToString());
                    //throw new ArgumentOutOfRangeException();
                    break;
            }
        ...
        {
            //Debug.LogMessage(LogEventLevel.Verbose, this, "PowerToggle pressed, _requestedPowerStatus: {0}", _requestedPowerStatus);
            switch (_requestedPowerStatus)
            {
                case PowerHandler.PowerStatusEnum.PowerOn:
                    PowerOff();
                ...
                default:
                    switch (_currentPowerStatus)
                    {
                        ...
                        default:
                            Debug.LogMessage(LogEventLevel.Debug, this, "PowerToggle() ERROR _currentPowerStatus: {0}, not handled", _currentPowerStatus.ToString());
                            //throw new ArgumentOutOfRangeException();
                            break;
                    }
                    break;
        ...
        private void GetPower()
        {
            try
            {
                ...
                Debug.LogMessage(LogEventLevel.Debug, this, "GetPower('{0}')", msg_);
                _commandQueue.Enqueue(new Commands.PJLinkCommand
                {
                    Coms = _coms,
                    Message = msg_
            ...
            {
                Debug.LogMessage(LogEventLevel.Warning, this, "GetPower ERROR: {0}", e.Message);
            }
        }
        ...
        public void ExecuteSwitch(object inputSelector)
        {
            ...
            {
                Debug.LogMessage(LogEventLevel.Debug, this, "Error executing switch : {0}{1}", ex.Message, ex.StackTrace);
            }
            finally
            {
                Feedbacks.FireAllFeedbacks();
        ...
        private void UpdateDriversRequiringAuthString(string authString, string classString)
        {
               ...
               if (DriversRequiringAuthString != null)
                {
                    ...
                    {
                        //Debug.LogMessage(LogEventLevel.Verbose, this, "HandleAuthStatusUpdated driver {0}", driver == null ? "== null" : driver.GetType().ToString());
                        if (authString != null)
            ...
            {
                Debug.LogMessage(LogEventLevel.Verbose, this, "UpdateDriversRequiringAuthString ERROR: {0}", e.Message);
            }
        }
        ...
        private void HandleClassUpdated(object sender, Events.StringEventArgs eventArgs)
        {
            try
            {
                //Debug.LogMessage(LogEventLevel.Debug, this, "HandleClassUpdated()");
                if (eventArgs == null)
                    Debug.LogMessage(LogEventLevel.Debug, this, "HandleClassUpdated eventArgs == null");
                else if (eventArgs.Val == null)
                    Debug.LogMessage(LogEventLevel.Debug, this, "HandleClassUpdated eventArgs.Val == null");
                else
                    Debug.LogMessage(LogEventLevel.Debug, this, "HandleClassUpdated({0})", eventArgs.Val);
            ...
            {
                Debug.LogMessage(LogEventLevel.Verbose, this, "HandleClassUpdated ERROR: {0}", e.Message);
            }
        }

        private void GetClass()
        {
            Debug.LogMessage(LogEventLevel.Verbose, this, "GetClass()");
            _commandQueue.Enqueue(new Commands.PJLinkCommand
            {
                Coms = _coms,
        ...
        private void HandleAuthUpdated(object sender, Events.StringEventArgs eventArgs)
        {
            try
            {
                if (eventArgs.Val == null)
                {
                    ...
                    Debug.LogMessage(LogEventLevel.Verbose, this, "HandleAuthUpdated, eventArgs.Val == null", eventArgs.Val);
                }
                else
            ...
            {
                Debug.LogMessage(LogEventLevel.Verbose, this, "HandleAuthUpdated ERROR: {0}", e.Message);
            }
        }
        ...
        private static CommunicationMonitorConfig GetDefaultMonitorConfig()
        {
            return new CommunicationMonitorConfig()
                {
                    PollInterval = 30000,
                    PollString = Commands.Protocol1 + Commands.Power + " ?\x0D",
